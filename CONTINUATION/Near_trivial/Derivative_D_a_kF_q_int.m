function [D_a_kF_j] = Derivative_D_a_kF_q_int(a_k,a_delta,epsilon,q,k,N,alpha,beta,u2,delta,Q)
% Correction for epsilon in desingularized map:
a_delta = a_delta;
% Case k = q
if k == q
    % Parameters
...
if k == q+1 || (k == 0 && q == Q)
    % First row
    D_a_kF_j(1,:) = [1,2*ones(1,N)].*(-1).^(1:N+1);
end
...
if k == mod(q + delta,Q+1) 
    ...
    % Preallocation
    D_a_kPsi_q = intval(zeros(N+2));
    a_k_ext = intval([a_k;zeros(N+2,1)]);
